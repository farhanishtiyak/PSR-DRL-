...
def extract_first_death_time(df, method_type):
    ...
    if method_type == 'competitor':
        ...
        # For EER-RL data, find when OperatingNodes first drops below 50
        if df is not None and 'OperatingNodes' in df.columns:
            first_death_idx = df[df['OperatingNodes'] < 50].index
            if len(first_death_idx) > 0:
...
def plot_first_node_death_comparison():
    ...
    if len(methods) > 0:
        ...
        ax.set_title('First Node Death Time Comparison\n50-Node Wireless Sensor Network', 
                    fontweight='bold', fontsize=14)
        ax.grid(True, alpha=0.3)
        
        # Add improvement percentage if both values exist
        if len(death_times) == 2:
            improvement = ((death_times[1] - death_times[0]) / death_times[0]) * 100
            ax.text(0.5, max(death_times) * 0.9, 
...
def plot_network_lifetime_comparison():
    ...
    if eerrl_df is not None and 'Times' in eerrl_df.columns and 'OperatingNodes' in eerrl_df.columns:
        # Convert OperatingNodes to percentage (assuming max is 50 nodes)
        eerrl_percentage = (eerrl_df['OperatingNodes'] / 50) * 100
...
def generate_summary_statistics():
    ...
    if eerrl_df is not None:
        ...
        final_percentage_eerrl = (final_nodes_eerrl / 50) * 100  # Assuming 50 nodes total
...
def main():
    ...
    print("Generating 50-Node Network Performance Comparison...")
    
    # Change to the correct directory
    os.chdir('/home/ishtiyak/Desktop/Thesis/MatLab/Comparison/50')

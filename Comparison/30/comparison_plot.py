...
def extract_first_death_time(df, method_type):
    ...
    if method_type == 'competitor':
        ...
        # For EER-RL data, find when OperatingNodes first drops below 30
        if df is not None and 'OperatingNodes' in df.columns:
            first_death_idx = df[df['OperatingNodes'] < 30].index
            if len(first_death_idx) > 0:
...
def plot_network_lifetime_comparison():
    ...
    if eerrl_df is not None and 'Times' in eerrl_df.columns and 'OperatingNodes' in eerrl_df.columns:
        # Convert OperatingNodes to percentage (assuming max is 30 nodes)
        eerrl_percentage = (eerrl_df['OperatingNodes'] / 30) * 100
...
def generate_summary_statistics():
    ...
    if eerrl_df is not None:
        ...
        final_percentage_eerrl = (final_nodes_eerrl / 30) * 100  # Assuming 30 nodes total
...
def main():
    ...
    print("Generating 30-Node Network Performance Comparison...")
    
    # Change to the correct directory
    os.chdir('/home/ishtiyak/Desktop/Thesis/MatLab/Comparison/30')

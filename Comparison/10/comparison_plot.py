...
def generate_summary_statistics():
    ...
    if proposed_first_death is not None and eerrl_first_death is not None:
        ...
        print(f"Improvement (EER-RL vs PSWR-DRL): {improvement_eerrl_vs_prop:+.2f}%")
    ...
    if network_df is not None:
        ...
        print(f"PSWR-DRL - Final time: {final_time_prop:.0f}, Final live %: {final_percentage_prop:.1f}%")
    
    if eerrl_df is not None:
        final_time_eerrl = eerrl_df['Times'].max()
        final_nodes_eerrl = eerrl_df['OperatingNodes'].iloc[-1]
        final_percentage_eerrl = (final_nodes_eerrl / 10) * 100  # Assuming 10 nodes total
...
def main():
    ...
    os.chdir('/home/ishtiyak/Desktop/Thesis/MatLab/Comparison/10')
    ...
    fig3 = plot_network_lifetime_comparison()
    fig3.savefig('overlay_comparison_all_methods_10nodes.png', dpi=300, bbox_inches='tight')
    print("✓ Overlay comparison saved as 'overlay_comparison_all_methods_10nodes.png'")

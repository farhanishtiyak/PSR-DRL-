#!/usr/bin/env python3
"""
Script to compare PSR-DRL vs RLBEEP vs EER-RL (30 Nodes)
"""
...
def plot_comparison(dql_csv, traditional_csv):
    ...
    plt.title('PSR-DRL - 30 Nodes', fontweight='bold')
    ...
    plt.xlabel('Epoch', fontsize=14)
    plt.ylabel('First Node Death Time (s)', fontsize=14)
    plt.title('RLBEEP - 30 Nodes', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper right')
    ...
    comparison_file = 'results/comparison_dql_vs_traditional_30nodes.png'
    ...
    overlay_file = 'results/overlay_comparison_30nodes.png'
    ...
    print("\n" + "="*60)
    print("PERFORMANCE SUMMARY - 30 Nodes (300 Epochs)")
    print("="*60)
    print(f"PSR-DRL:")
...
def plot_comparison_with_eerrl(dql_csv, traditional_csv, eerrl_csv):
    ...
    overlay_file = 'results/overlay_comparison_all_methods_30nodes.png'
    overlay_file_pdf = 'results/overlay_comparison_all_methods_30nodes.pdf'
    plt.savefig(overlay_file, dpi=300, bbox_inches='tight')
...
def main():
    ...
    if os.path.exists(results_dir):
        for file in os.listdir(results_dir):
            if '30nodes' in file and file.endswith('.csv'):

#!/usr/bin/env python3
"""
Simple Epoch Runner for 50-Node RLBEEP
...
def plot_first_death_times(results, method_name):
    ...
    plt.title(f'First Node Death Time vs Epoch\n{method_name} - 50 Nodes', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=10)
    
    # Add statistics text box
    stats_text = f"""Statistics (50 Nodes):
...
Valid epochs: {len(death_times)}/300"""
    ...
    # Save the plot
    plot_filename = f"results/{method_name.lower().replace(' ', '_')}_50nodes_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
...
def run_dql_enhanced_epochs():
    ...
    print("Starting DQL-Enhanced RLBEEP Epochs (50 Nodes)...")
    print("="*60)
    ...
    filename = f"dql_enhanced_rlbeep_first_death_times_50nodes_{timestamp}.csv"
    ...
    if valid_results:
        ...
        print(f"\nDQL-Enhanced RLBEEP Results (50 Nodes):")
...
def run_traditional_epochs():
    ...
    print("Starting Traditional RLBEEP Epochs (50 Nodes)...")
    print("="*60)
    ...
    # Save results to CSV
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    filename = f"traditional_rlbeep_first_death_times_50nodes_{timestamp}.csv"
    csv_path = save_results_to_csv(results, filename)
    ...
    if valid_results:
        death_times = [r['first_death_time'] for r in valid_results]
        print(f"\nTraditional RLBEEP Results (50 Nodes):")
        print(f"Mean first death time: {np.mean(death_times):.1f} seconds")
        print(f"Standard deviation: {np.std(death_times):.1f} seconds")
        print(f"Valid epochs: {len(valid_results)}/300")
...
def main():
    ...
    print("RLBEEP Epoch Runner - 50 Nodes")

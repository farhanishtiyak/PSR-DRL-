#!/usr/bin/env python3
"""
Simple script to plot first node death times from CSV file (50 Nodes)
"""
...
def plot_first_death_times(csv_file):
    ...
    plt.title(f'First Node Death Time vs Epoch\n{method} - 50 Nodes', fontsize=14, fontweight='bold')
...
def main():
    ...
    if os.path.exists(results_dir):
        for file in os.listdir(results_dir):
            if file.endswith('.csv') and 'first_death_times' in file and '50nodes' in file:
                csv_files.append(os.path.join(results_dir, file))
    
    if not csv_files:
        print("No 50-node CSV files found in results directory!")

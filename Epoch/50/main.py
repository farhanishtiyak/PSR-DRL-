...
NUM_NODES = 50
NUM_CLUSTERS = 20
SEND_RANGE = 25  # meters
MAX_LONGITUDE = 150.0  # meters
MAX_LATITUDE = 150.0  # meters

# Energy parameters (adjusted for more realistic WSN behavior)
NODE_INITIAL_ENERGY = 200  # Joules (to match competitor's 200 ± 15 range)
POWER_SEND = 0.4  # Joules per transmission (slightly increased for 50-node network)
POWER_RECEIVE = 0.25  # Joules per reception (slightly increased for 50-node network)
POWER_ACTIVE = 0.12  # Joules per second for active nodes (slightly increased for 50-node network)
POWER_SLEEP = 0.06  # Joules per second for sleeping nodes (slightly increased for 50-node network)
...
SLEEP_RESTRICT_THRESHOLD = 10  # Base threshold - matches simulation.py
...
CHANGE_THRESHOLD = 10.0  # Base threshold - matches simulation.py
TRANSMISSION_PERIOD = 6  # seconds (from dataset)

# Simulation parameters
TOTAL_SIMULATION_TIME = 4000  # seconds
...
class DatasetLoader:
    ...
    def get_sensor_reading(self, node_id, time_index, column='temperature_mean'):
        """Get a sensor reading for a specific node at a specific time index with enhanced data generation for 50 nodes"""
        ...
        if node_id not in self.node_data or self.node_data[node_id] is None:
            # Enhanced data generation for 50-node network

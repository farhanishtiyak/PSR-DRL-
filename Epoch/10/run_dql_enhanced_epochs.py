#!/usr/bin/env python3
"""
RLBEEP DQL-Enhanced Epoch Runner - 10 Nodes
============================================

This script runs the DQL-Enhanced RLBEEP simulation for 300 epochs and collects
...
def run_epoch_simulation(epoch_num, main_module):
    """
    Run a single epoch of the DQL-Enhanced RLBEEP simulation
    
    Args:
        epoch_num (int): Current epoch number
        main_module: The loaded main module
        
    Returns:
        dict: Results from this epoch
    """
    print(f"Running DQL-Enhanced RLBEEP Epoch {epoch_num}/300...")
    ...
    try:
        ...
        total_transmissions = results.get('total_transmissions', 0)
        avg_energy_remaining = results.get('avg_energy_remaining', 0.0)
        ...
        energy_efficiency = 1.0 - (avg_energy_remaining / main_module.NODE_INITIAL_ENERGY) if avg_energy_remaining > 0 else 1.0
        ...
        print(f"Epoch {epoch_num} completed - First death: {first_death_time}s, Survival: {final_live_percentage:.1f}%")
...
def save_epoch_results(results_list, filename):
    ...
    # Define fieldnames
    fieldnames = ['epoch', 'first_death_time', 'final_live_percentage', 'simulation_duration', 
                  'total_transmissions', 'energy_efficiency', 'method']
...
def plot_first_death_time_epochs(results_list, save_path):
    ...
    # Plot data with different colors for no-death cases
    no_death_mask = np.array(death_times) >= 3000
    death_times_plot = np.array(death_times)
    
    # Plot normal deaths
    if np.any(~no_death_mask):
        normal_epochs = np.array(epochs)[~no_death_mask]
        normal_deaths = death_times_plot[~no_death_mask]
        plt.plot(normal_epochs, normal_deaths, 'b-', alpha=0.8, linewidth=2, label='First Death Time')
        plt.scatter(normal_epochs, normal_deaths, c='red', s=35, alpha=0.8, zorder=5, 
                   edgecolors='darkred', linewidth=0.5)
    
    # Plot no-death cases
    if np.any(no_death_mask):
        no_death_epochs = np.array(epochs)[no_death_mask]
        no_death_times = death_times_plot[no_death_mask]
        plt.scatter(no_death_epochs, no_death_times, c='green', s=60, marker='^', 
                   label='No Deaths', zorder=6, edgecolors='darkgreen', linewidth=1)
    
    # Formatting
    plt.xlabel('Epoch Number', fontsize=16, fontweight='bold')
    plt.ylabel('First Node Death Time (seconds)', fontsize=16, fontweight='bold')
    plt.title('DQL-Enhanced RLBEEP - First Node Death Time vs Epochs\n10-Node Network', 
    ...
    y_min = min(death_times) * 0.95 if min(death_times) < 3000 else 1000
    plt.ylim(y_min, 3100)  # Slight padding above max time
    ...
    no_death_count = len([d for d in death_times if d >= 3000])
    ...
    if valid_deaths:
        ...
        stats_text = (f'Total Epochs: {len(epochs)}\n'
                     f'With Deaths: {len(valid_deaths)}\n'
                     f'No Deaths: {no_death_count}\n'
    ...
    # Method info
    method_info = f'Method: DQL-Enhanced RLBEEP\nNodes: 10\nTotal Time: 3000s'
...
def main():
    """Main function to run 300 epochs of DQL-Enhanced RLBEEP simulation"""
    print("="*60)
    print("   DQL-ENHANCED RLBEEP - 300 EPOCH ANALYSIS")
    ...
    for epoch in range(1, 301):
        epoch_result = run_epoch_simulation(epoch, main_module)
    ...
    csv_filename = f"dql_enhanced_rlbeep_epochs_300_{timestamp}.csv"
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    results_dir = os.path.join(current_dir, "results")
    plot_filename = f"dql_enhanced_rlbeep_first_death_vs_epochs_{timestamp}.png"
    ...
    final_survivals = [r['final_live_percentage'] for r in results_list]
    energy_efficiencies = [r['energy_efficiency'] for r in results_list]
    ...
    print(f"Average Network Survival: {np.mean(final_survivals):.1f}%")
    print(f"Average Energy Efficiency: {np.mean(energy_efficiencies)*100:.1f}%")

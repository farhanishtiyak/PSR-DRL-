...
class RLBEEPSimulation:
    ...
    def __init__(self, dataset_path, num_nodes=10, num_clusters=4, 
                 max_longitude=60.0, max_latitude=60.0, 
                 send_range=10, alpha=0.5,
        ...
        self.simulation_data = []  # For detailed time-series data
        self.total_energy_data = []
        self.packets_received_data = []
    ...
    def load_dataset(self):
        """Load the dataset for all nodes"""
        dataset = {}
    ...
    def setup_network(self):
        ...
        for i in range(1, self.num_nodes + 1):
            ...
            # Add randomness to initial energy (between 95 and 105)
            initial_energy = 100 + random.uniform(-5, 5)
            self.nodes.append(Node(i, x, y, initial_energy))
        
        # Select cluster heads (for simplicity, we select the first num_clusters nodes)
        for i in range(self.num_clusters):
            if i < len(self.nodes):
                node = self.nodes[i]
                # Replace regular node with cluster head
                cluster_head = ClusterHead(node.node_id, node.x, node.y)
    ...
    def get_sensor_data(self, node_id, time_index):
        ...
        dataset_size = len(self.dataset[node_id])
        if dataset_size == 0:
        ...
        # This affects data transmission decisions and energy consumption
        base_value = float(self.dataset[node_id].iloc[actual_index, temperature_column])
    ...
    def run_simulation(self):
        ...
        for t in range(self.total_time):
            ...
            # Calculate total energy remaining
            total_energy = sum(node.energy for node in self.nodes if node.is_alive())
            self.total_energy_data.append(total_energy)
            
            # Track packets received at sink
            packets_received = self.sink.received_packets
            self.packets_received_data.append(packets_received)
            ...
            self.simulation_data.append({
                'Time': t,
                'Live_Node_Percentage': live_percentage,
    ...
    def save_csv_files(self):
        """Save simulation data and summary to CSV files"""
        
        # Save detailed simulation data
        data_filename = 'rlbeep_simulation_data.csv'
        with open(data_filename, 'w', newline='') as csvfile:
            fieldnames = ['Time', 'Live_Node_Percentage', 'Total_Energy', 'Packets_Received', 'Active_Nodes']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(self.simulation_data)
        
        print(f"Detailed simulation data saved to: {data_filename}")
        ...
        summary_data = [
            ...
            ['Network_Area', f'{self.max_longitude}x{self.max_latitude}'],
            ['Send_Range', self.send_range],
            ['Total_Time', self.total_time],
            ['First_Death_Time', self.first_death_time if self.first_death_time != -1 else 'No deaths'],
            ['Final_Live_Percentage', f"{self.live_node_percentage[-1]:.1f}" if self.live_node_percentage else "0.0"]
        ]
    ...
    def select_new_cluster_head(self, old_ch):
        ...
        for node in candidates:
            ...
            energy_score = node.energy / 100  # Normalize by initial energy
...
def main():
    ...
    simulation = RLBEEPSimulation(
        ...
        ch_rotation_interval=200  # Rotate cluster heads every 300 seconds
    )
    
    results = simulation.run_simulation()
    simulation.plot_results()
    simulation.save_csv_files()

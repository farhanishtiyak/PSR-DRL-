...
NUM_NODES = 10
NUM_CLUSTERS = 4
SEND_RANGE = 10  # meters
MAX_LONGITUDE = 60.0  # meters
MAX_LATITUDE = 60.0  # meters
...
POWER_ACTIVE = 0.1  # Joules per second for active nodes
POWER_SLEEP = 0.05  # Joules per second for sleeping nodes (0.5% of active - very low)
...
CHANGE_THRESHOLD = 1.5  # Base threshold - nodes will vary this (reduced from 2.0)
TRANSMISSION_PERIOD = 6  # seconds (from dataset)

# Simulation parameters
TOTAL_SIMULATION_TIME = 3000  # seconds
...
class DatasetLoader:
    ...
    def load_node_data(self, node_id):
        ...
        if os.path.exists(file_path):
            ...
            # Only print a warning for nodes that should exist based on configuration
    ...
    def get_sensor_reading(self, node_id, time_index, column='temperature_mean'):
        """Get a sensor reading for a specific node at a specific time index"""
        ...
        if node_id not in self.node_data or self.node_data[node_id] is None:
            return random.uniform(20, 30)  # Return random value if data not available
...
class RLBEEPSimulation:
    ...
    def __init__(self, dataset_path, num_nodes=NUM_NODES, num_clusters=NUM_CLUSTERS, 
        ...
        self.node_death_times = {}  # Track individual node death times
        self.node_lifetime_data = []  # Detailed node information over time
    ...
    def update_statistics(self):
        ...
        # Track node death times
        for node in self.nodes:
            if not node.is_alive() and node.node_id not in self.node_death_times:
                self.node_death_times[node.node_id] = self.time
        
        # Create detailed node information snapshot
        node_data = {
            'time': self.time,
        ...
        # Add individual node energy and status
        for i, node in enumerate(self.nodes, 1):
            node_data[f'node_{i}_energy'] = node.energy
            node_data[f'node_{i}_alive'] = 1 if node.is_alive() else 0
            node_data[f'node_{i}_type'] = node.type.name if hasattr(node.type, 'name') else str(node.type)
        
        self.node_lifetime_data.append(node_data)
    ...
    def save_csv_results(self):
        ...
        summary_data = {
            'Parameter': ['Nodes', 'Clusters', 'Simulation_Duration', 'First_Death_Time', 
                         'Final_Live_Percentage', 'Total_Transmissions', 'Avg_Energy_Remaining', 'Energy_Efficiency'],
            'Value': [
                self.num_nodes,
                self.num_clusters,
                self.time,
                self.first_death_time if self.first_death_time != -1 else 'No deaths',
                self.live_node_percentage[-1] if self.live_node_percentage else 0,
                sum(self.transmissions) if self.transmissions else 0,
                sum(self.energy_levels[-1]) / len(self.energy_levels[-1]) if self.energy_levels and self.energy_levels[-1] else NODE_INITIAL_ENERGY,
                1 - (sum(self.energy_levels[-1]) / len(self.energy_levels[-1]) / NODE_INITIAL_ENERGY) if self.energy_levels and self.energy_levels[-1] else 0
            ]
        }
...
class SimulationVisualizer:
    ...
    def plot_energy_levels(self, filename="energy_levels.png"):
        """Plot the energy levels of all nodes"""
        plt.figure(figsize=(12, 6))
        
        # Create time array
        times = list(range(len(self.simulation.energy_levels)))
        
        # Plot energy level for each node
        for i in range(len(self.simulation.nodes)):
            node = self.simulation.nodes[i]
            energy_data = [levels[i] if i < len(levels) else 0 
                          for levels in self.simulation.energy_levels]
            
            if node.type == NodeType.CLUSTER_HEAD:
                plt.plot(times, energy_data, 'r-', alpha=0.7)
            else:
                plt.plot(times, energy_data, 'b-', alpha=0.3)
        
        # Add average energy line
        avg_energy = [sum(levels) / len(levels) for levels in self.simulation.energy_levels]
        plt.plot(times, avg_energy, 'k-', linewidth=2, label='Average Energy')
        
        plt.title("Node Energy Levels Over Time")
        plt.xlabel("Time (seconds)")
        plt.ylabel("Energy (J)")
        plt.grid(True)
        plt.legend(['Cluster Heads', 'Regular Nodes', 'Average'])
    ...
    def plot_data_transmission(self, filename="data_transmission.png"):
        """Plot data transmission statistics"""
        plt.figure(figsize=(10, 6))
        
        # Create time array
        times = list(range(len(self.simulation.transmissions)))
        
        # Plot number of transmissions per time step
        plt.plot(times, self.simulation.transmissions, 'b-', alpha=0.5)
        
        # Add moving average line
        window_size = 50
        moving_avg = []
        for i in range(len(self.simulation.transmissions)):
            start_idx = max(0, i - window_size + 1)
            window = self.simulation.transmissions[start_idx:i+1]
            moving_avg.append(sum(window) / len(window))
        
        plt.plot(times, moving_avg, 'r-', linewidth=2, label='Moving Average')
        
        plt.title("Data Transmissions Over Time")
    ...
    def create_results_dashboard(self, filename="rlbeep_results_dashboard.png"):
        ...
        for node in self.simulation.nodes:
            if node.type == NodeType.REGULAR:
                ...
                if node.cluster_id is not None:
                    ch = self.simulation.cluster_heads[node.cluster_id]
                    plt.plot([node.x, ch.x], [node.y, ch.y], 'b-', alpha=0.2)
        
        for ch in self.simulation.cluster_heads:
            plt.scatter(ch.x, ch.y, color='red', s=80, marker='s')
            if ch.distance_to(self.simulation.sink) <= self.simulation.send_range:
                plt.plot([ch.x, self.simulation.sink.x], 
                         [ch.y, self.simulation.sink.y], 'g-', alpha=0.3)
        ...
        if self.simulation.first_death_time != -1:
            plt.axvline(x=self.simulation.first_death_time, color='r', linestyle='--')
            plt.text(self.simulation.first_death_time + 10, 50, 
                    f'First Death: {self.simulation.first_death_time}s', 
                    fontsize=9)
        
        plt.title("Node Lifetime")
        plt.xlabel("Time (seconds)")
        plt.ylabel("Alive Nodes (%)")
        plt.grid(True, alpha=0.3)
        plt.ylim(0, 105)
        ...
        # Average energy line
        avg_energy = [sum(levels) / len(levels) for levels in self.simulation.energy_levels]
        plt.plot(times, avg_energy, 'k-', linewidth=2, label='Average')
        
        # Min/max energy
        min_energy = [min(levels) for levels in self.simulation.energy_levels]
        max_energy = [max(levels) for levels in self.simulation.energy_levels]
        
        plt.fill_between(times, min_energy, max_energy, alpha=0.2, color='blue',
                        label='Min-Max Range')
        ...
        plt.plot(times, moving_avg, 'r-', linewidth=2, label='Moving Avg')
        plt.fill_between(times, [0] * len(times), self.simulation.transmissions, 
                        alpha=0.2, color='blue')
        
        plt.title("Data Transmissions")
        plt.xlabel("Time (seconds)")
        plt.ylabel("Number of Transmissions")
        plt.grid(True, alpha=0.3)
        plt.legend()
        ...
        # Add overall title
        plt.suptitle(f"RLBEEP Simulation Results\n{self.simulation.num_nodes} Nodes, "
                   f"{self.simulation.num_clusters} Clusters, {self.simulation.total_time}s", 
                   fontsize=16, y=0.98)
    ...
    def plot_all_results(self):
        """Plot required visualizations only"""
        ...
        try:
            # Only generate the two required PNG files
            self.plot_network_topology()
            self.plot_node_lifetime()
            print("Visualizations saved to results directory")
            
            # Verify the files were created
            expected_files = ["network_topology.png", "node_lifetime.png"]
            missing_files = []
            ...
            if missing_files:
                print(f"Warning: Some visualization files were not created: {missing_files}")
                print(f"Check permissions and available disk space in {self.results_dir}")
        except Exception as e:
...
def main():
    ...
    # Adjust number of nodes if necessary
    actual_nodes = min(args.nodes, available_nodes)
    if actual_nodes < args.nodes:
        print(f"WARNING: Only {actual_nodes} nodes available in dataset. Adjusting simulation parameters.")
    ...
    if actual_clusters < args.clusters:
        print(f"WARNING: Reducing clusters to {actual_clusters} due to node availability.")

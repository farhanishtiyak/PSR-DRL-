#!/usr/bin/env python3
"""
Script to compare PSR-DRL vs RLBEEP vs EER-RL (100 Nodes)
"""
...
def plot_comparison(dql_csv, traditional_csv):
    ...
    plt.title('PSR-DRL - 100 Nodes', fontweight='bold')
    ...
    plt.xlabel('Epoch', fontsize=14)
    plt.ylabel('First Node Death Time (s)', fontsize=14)
    plt.title('RLBEEP - 100 Nodes', fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(loc='upper right')
    ...
    comparison_file = 'results/comparison_dql_vs_traditional_100nodes.png'
    ...
    overlay_file = 'results/overlay_comparison_100nodes.png'
    ...
    print("\n" + "="*60)
    print("PERFORMANCE SUMMARY - 100 Nodes (300 Epochs)")
    print("="*60)
    print(f"PSR-DRL:")
...
def plot_comparison_with_eerrl(dql_csv, traditional_csv, eerrl_csv):
    ...
    overlay_file = 'results/overlay_comparison_all_methods_100nodes.png'
    overlay_file_pdf = 'results/overlay_comparison_all_methods_100nodes.pdf'
    plt.savefig(overlay_file, dpi=300, bbox_inches='tight')
    ...
    if os.path.exists(results_dir):
        for file in os.listdir(results_dir):
            if file.endswith('.csv'):
                if 'dql_enhanced' in file and '100nodes' in file:
                    dql_file = os.path.join(results_dir, file)
                elif 'traditional' in file and '100nodes' in file:
                    traditional_file = os.path.join(results_dir, file)
                elif 'eer_rl' in file and '100nodes' in file:

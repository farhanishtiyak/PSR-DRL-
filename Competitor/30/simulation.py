...
class RLBEEPSimulation:
    ...
    def __init__(self, dataset_path, num_nodes=30, num_clusters=12, 
                 max_longitude=100.0, max_latitude=100.0, 
                 send_range=20, alpha=0.5,  # Increased send_range for larger network
    ...
    def setup_network(self):
        ...
        for i in range(1, self.num_nodes + 1):
            ...
            # Add randomness to initial energy (increased for larger network)
            # With 30 nodes, increase base energy to handle more traffic
            initial_energy = 150 + random.uniform(-10, 10)  # Range: 140-160
    ...
    def get_sensor_data(self, node_id, time_index):
        ...
        # Map nodes 1-10 to their actual dataset, nodes 11-30 to reused data with variations
        if node_id <= 10:
            # Use original dataset for nodes 1-10
            dataset_node_id = node_id
        else:
            # For nodes 11-30, map to nodes 1-10 cyclically
        ...
        # Add variation based on node_id for nodes 11-30 to create diversity
        if node_id > 10:
            # Create different temperature patterns for extended nodes
            node_offset = (node_id - 11) * 0.3  # Each extended node has slight offset
            seasonal_variation = math.sin((time_index / 100) + node_offset) * 1.5  # Seasonal pattern
    ...
    def select_new_cluster_head(self, old_ch):
        ...
        for node in candidates:
            ...
            energy_score = node.energy / 150  # Normalize by updated initial energy
...
def main():
    ...
    # Create and run simulation with 30 nodes and 12 cluster heads
    simulation = RLBEEPSimulation(
        dataset_path=dataset_path,
        num_nodes=30,  # Increased from 10 to 30
        num_clusters=12,  # Increased from 4 to 12
        max_longitude=100.0,  # Increased area to accommodate more nodes
        max_latitude=100.0,   # Increased area to accommodate more nodes
        send_range=20,        # Increased range for larger network (was 15)
        alpha=0.5,
        dfr_min=5.0,
        dfr_max=55.0,
        total_time=3000,
        ...
        ch_rotation_interval=200  # Rotate cluster heads every 200 seconds
    )

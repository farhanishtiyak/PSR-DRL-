...
class Node:
    ...
    def reduce_energy(self, activity_type):
        ...
        base_energy_consumption = {
            'send': 0.4,      # Slightly increased for 50-node network
            'receive': 0.25,  # Slightly increased for 50-node network
            'active': 0.12,   # Slightly increased for 50-node network
            'sleep': 0.06     # Slightly increased for 50-node network
        }
...
class RLBEEPSimulation:
    ...
    def __init__(self, dataset_path, num_nodes=50, num_clusters=20, 
                 max_longitude=150.0, max_latitude=150.0, 
                 send_range=25, alpha=0.5,  # Increased parameters for 50 nodes
    ...
    def setup_network(self):
        ...
        for i in range(1, self.num_nodes + 1):
            ...
            # Add randomness to initial energy (increased for 50-node network)
            # With 50 nodes, increase base energy to handle more traffic
            initial_energy = 200 + random.uniform(-15, 15)  # Range: 185-215
    ...
    def get_sensor_data(self, node_id, time_index):
        ...
        # Map nodes 1-10 to their actual dataset, nodes 11-50 to reused data with variations
        if node_id <= 10:
            # Use original dataset for nodes 1-10
            dataset_node_id = node_id
        else:
            # For nodes 11-50, map to nodes 1-10 cyclically
        ...
        # Add variation based on node_id for nodes 11-50 to create diversity
        if node_id > 10:
            # Create different temperature patterns for extended nodes
            node_offset = (node_id - 11) * 0.2  # Smaller offset for more nodes
            seasonal_variation = math.sin((time_index / 120) + node_offset) * 2.0  # Varied seasonal pattern
            daily_variation = math.cos((time_index / 24) + node_offset * 0.5) * 0.8  # Daily temperature cycle
            base_value += seasonal_variation + daily_variation
    ...
    def process_packet_reception(self, node, packet):
        ...
        if packet.hops > 10:  # Maximum 10 hops allowed
            return False
    ...
    def select_new_cluster_head(self, old_ch):
        ...
        for node in candidates:
            ...
            energy_score = node.energy / 200  # Normalize by updated initial energy for 50 nodes
...
def main():
    ...
    # Create and run simulation with 50 nodes and 20 cluster heads
    simulation = RLBEEPSimulation(
        dataset_path=dataset_path,
        num_nodes=50,  # Increased from 30 to 50
        num_clusters=20,  # Increased from 12 to 20
        max_longitude=150.0,  # Increased area to accommodate more nodes
        max_latitude=150.0,   # Increased area to accommodate more nodes
        send_range=25,        # Increased range for larger network (was 20)
        alpha=0.5,
        dfr_min=5.0,
        dfr_max=55.0,
        total_time=3000,
        ...
        ch_rotation_interval=250  # Slightly increased rotation interval for more nodes
    )
